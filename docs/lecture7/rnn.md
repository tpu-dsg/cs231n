...
- **非線形（$\tanh$）の除去**：消失勾配問題を解決するために非線形性（$\tanh$）を取り除くと、 $\frac{\partial L_{t}}{\partial W} = \frac{\partial L_{t}}{\partial h_{t}}(\prod_{t=2}^{T} W_{hh}^{T-1})\frac{\partial h_{1}}{\partial W}$ のようになります。
    - 勾配爆発： $W_{hh}$ の最大特異値が1を超えた場合、勾配が爆発し将来のタイムステップから非常に大きな勾配が戻ってくることになります。勾配の爆発は、しばしば非数の勾配を得ることにつながります。
    - 勾配消失: $W_{hh}$ の最大特異値が1より小さい場合、前述の勾配消失問題が発生し、学習が著しく遅くなります。
